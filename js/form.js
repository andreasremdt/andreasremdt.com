(function() {
  ...
  function handleFormError(err) {
    showPopup(`Sorry, your email could not be sent. Please try again later or use the direct email address.`, true);
  ...
  function showPopup(message, isError = false) {
    var text = form.querySelector("[data-form-error]");

    if (!text) {
      text = document.createElement('p');
  
      text.setAttribute("data-form-error", true);
      text.classList.add("form-message", isError ? "is-error" : "is-success");
      text.innerHTML = `<b>${isError ? "Error!" : "Success!"}</b> ${message}`;
